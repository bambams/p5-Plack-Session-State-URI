...
sub log_result {
    ...
    unless ($log_notice) {
        if (defined $log_fh) {
            warn "More verbose test results are being logged to $log_fn.\n";
            binmode $log_fh, ':utf8';
        } else {
            warn "Failed to create temporary log file. ",
                    "I would have logged more verbose to it.\n";
        }
    ...
    for(my ($i, $l) = (0, scalar @test_params); $i<$l; $i+=2) {
        print $log_fh "$test_params[$i]: $test_params[$i+1]\n";
    }
...
sub test_lambda {
    my ($desc, $lambda, %named_params) = @_;

    my @test_params;

    my $result = $lambda->($_, \@test_params);
    ...
    test_result($desc, $result, %named_params);
}

sub test_match {
    my ($desc, $re, %named_params) = @_;
